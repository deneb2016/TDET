...
from model.sm_mcl_tdet_vgg16 import SM_MCL_TDET_VGG16
...
def parse_args():
    ...
    parser.add_argument('--net', default='SM_MCL_TDET_VGG16', type=str)
...
def train():
    args = parse_args()
    assert args.num_group == 60 or args.group_method != 'cls'
    ...
    for step in range(args.start_iter, args.max_iter + 1):
        ...
        if step % args.disp_interval == 0:
            ...
            # attention_weight = str(F.softmax(model.attention_layer.weight.clone(), 0))
            # print(attention_weight)
            # log_file.write(attention_weight + '\n')
        ...
        if step % args.save_interval == 0 or step == args.max_iter:
            ...
            checkpoint['num_group'] = args.num_group
