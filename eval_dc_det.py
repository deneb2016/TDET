...
def eval():
    ...
    if checkpoint['net'] == 'DC_VGG16_DET':
        model = DC_VGG16_DET(DC_VGG16_CLS(None, 20, 3, 4))
    ...
    for index in range(len(test_dataset)):
        ...
        for h_flip, im_size in comb:
            ...
            local_scores = np.zeros(shape=[proposals.size(0), 20])
            for cls in range(20):
                here_score = model(im_data, cls, proposals)
                local_scores[:, cls] = here_score.detach().cpu().numpy()
        ...
        for cls in range(20):
            if cls not in test_batch['gt_labels']:
                continue
