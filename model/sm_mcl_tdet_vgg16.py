...
class SM_MCL_TDET_VGG16(nn.Module):
    ...
    def _init_weights(self):
        ...
        normal_init(self.cls_layer[-1], 0, 0.01, False)
        normal_init(self.det_layer[-1], 0, 0.01, False)
        normal_init(self.back_layer, 0, 0.01, False)
    ...
    def forward_det_only(self, im_data, rois, use_softmax=False, objectness_labels=None):
        N = rois.size(0)
        shared_feat = self.forward_shared_feat(im_data, rois)
        det_score = self.det_layer(shared_feat)
        back_score = self.back_layer(shared_feat)
        groups = det_score.new_zeros(size=[N], dtype=torch.long)
        if use_softmax:
            all_score = torch.cat([back_score, det_score], 1)
            pos_mask = objectness_labels.eq(1)
            _, max_score_indices = torch.max(det_score, 1)
            max_score_indices = max_score_indices + 1
            groups[pos_mask] = max_score_indices[pos_mask]
            loss = F.cross_entropy(all_score, groups)
        else:
            det_score = torch.exp(det_score)
            back_score = torch.exp(back_score)
            det_score = det_score / (det_score + back_score)
            objectness_labels = objectness_labels.view(N, 1).expand(det_score.size())
            loss = F.binary_cross_entropy(det_score, objectness_labels.to(torch.float32))
        return loss, groups
